    ...
    function Fetch(General2, FetchSet2, Process2, Transl2, Lib2, md52) {
        return class FetchData {
            ...
            constructor() {
                ...
                this.specialLinkParse = data => {
                    ...
                    try {
                        ...
                        nodes = [...nodes];
                        Lib2.log("specialLinkParse DOM", domBody, {
                            dev: General2.Dev,
                            collapsed: false
                        });
                        for (const [index, node] of nodes.entries()) {
                            const tag = node.tagName;
                            Lib2.log("specialLinkParse node", {
                                tag: tag,
                                content: node.$text()
                            }, {
                                dev: General2.Dev
            ...
            async _fetchContent(homeData) {
                ...
                Lib2.log("Fetch Content", homeData, {
                    dev: General2.Dev
                });
                if (Process2.IsNeko) {
                    let taskCount = 0;
                    ...
                    if (homeJson) {
                        if (this.metaDict.size === 0) {
                            ...
                            this.metaDict.set(Transl2("建立時間"), Lib2.getDate("{year}-{month}-{date} {hour}:{minute}"));
                            this.metaDict.set(Transl2("獲取頁面"), this.sourceURL);
                            Lib2.log("Meta Data", this.metaDict, {
                                dev: General2.Dev
                            });
                        }
                        {
