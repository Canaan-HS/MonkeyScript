...
(async () => {
    ...
    class Download {
        ...
        async ResetButton() {
            ...
            lock = false;
            const Button = Syn.$q("#File_Span button");
    ...
    (new class Main {
        ...
        async ButtonCreation() {
            ...
            Syn.WaitElem(".post__body h2, .scrape__body h2", null, {raf: true, all: true}).then(Files => {
                ...
                try {
                    ...
                    const spanElement = Syn.$createElement("span", { id: "File_Span" });

                    Syn.$createElement(spanElement, "svg", { // 創建設置 SVG
                    ...
                    Syn.$createElement(spanElement, "button", { // 創建 Button
                        class: "Download_Button",
                        text: lock ? Lang.Transl("下載中鎖定") : ModeDisplay,
                        disabled: lock
                    });

                    Files[0].appendChild(spanElement);
                    spanElement.$one("click", event => {
                        const target = event.target;

                        if (target.tagName === "BUTTON") {
                            let Instantiate = null;
                            Instantiate = new Download(CompressMode, ModeDisplay, target);
        ...
        async Injection() {
            ...
            /* 注銷菜單 */
            async function unregisterMenu() {
                for (let i = 1; i <= 2; i++) GM_unregisterMenuCommand("Menu-" + i);
            };
            ...
            async function registerMenu(Page) {
                ...
                if (self.Content(Page)) {
                    ...
                    Syn.Menu({
                        ...
                        [Lang.Transl("📃 開啟當前頁面帖子")]: { func: () => self.OpenAllPages() }
                    });
                }
            };
