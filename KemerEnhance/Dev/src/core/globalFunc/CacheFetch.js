...
export default async function CacheReq() {
    ...
    const saveCache = Lib.debounce(() => {
        Parame.DB.set(cacheKey, cache, { expireStr: "10m" }); // 有效 10 分鐘緩存 (每次都刷新)
    }, 1e3);
    ...
    async function fetchWrapper(windowContext, ...args) {
        ...
        if (response.status === 200 && (url.includes('api') || url.includes('default_config'))) {
            ...
            clone.text().then(bodyText => {
                if (bodyText) {
                    cache.set(url, { body: bodyText, status: clone.status, headers: clone.headers });
                    saveCache();
                }
            }).catch(() => { });
        }
            ...
            if (canCache) {
                this.addEventListener('load', () => {
                    if (this.status === 200 && this.responseText) {
                        // cache.set(url, this.responseText);
                        Lib.log(typeof this.responseText);
                    }
                });
            }
