...
| `BetterPostCard`  | Pixiv 名でサイト表示名を修正、カスタム可能、元サイトのタグへ直接リンク |     `true`     |
...
**リリースバージョン：2025.09.26-Beta**

### **更新内容**
1. ツールライブラリを更新
2. 名前を変更：FixArtist -> BetterPostCard
3. BetterPostCard に `previewAbove` 設定を追加。デフォルトは上に表示、false にすると下に表示される
4. BetterThumbnail を追加。attachments タイプを直接表示（nekohouse は未対応）【実験的】
