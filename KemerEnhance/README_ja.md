...
|    `BackToTop`    |                 投稿ページ移動後、自動的にトップへ戻る                 |     `true`     |
...
1. **互換性修正**  
   - サイトの最近の変更に合わせて調整  
2. **メニュー調整**  
   - 純粋なネイティブ実装に変更し、jQuery を削除  
3. **OriginalImage**  
   - 読み込みインジケーターのスタイルを調整  
4. **CardZoom 機能**  
   - モード1・2のスタイルを変更  
   - モード3を追加  
5. **BetterThumbnail（実験的）**  
   - 強化されたサムネイル機能を追加  
6. **FixArtist & LinkBeautify**  
   - 一部実装を最適化し、元のページ構造への破壊を軽減。オーバーレイ方式で元の外観を保持  
7. **TextToLink**  
   - 簡単な形式の Mega リンク修復を追加。パスワード断片を自動補完（複雑な形式は非対応）  
