...
**Релиз: 2025.09.07-Beta**

### **Изменения**
1. **OriginalImage**
   - Исправлена проблема с переключением страниц
   - Логика mode 1 изменена: теперь пакетная загрузка до 7 изображений за раз (без учёта ошибок, экспериментально)
   - Уменьшено количество автоматических повторных попыток при сбое загрузки (часто сервер просто не отдаёт оригиналы)
